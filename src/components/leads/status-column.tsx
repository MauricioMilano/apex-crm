  ...
  return (
    <div className="flex flex-col w-72 shrink-0">
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 mb-2">
        ...
        <span className="font-semibold text-sm text-foreground/90 flex-1 truncate">
          {status.name}
        </span>
        <Badge
          variant="secondary"
          className="bg-muted text-muted-foreground border-0 text-xs px-2 h-5"
        >
          {leads.length}
        </Badge>
      </div>
      ...
      <div
        ...
        className={cn(
          ...
          isDragOver
            ? 'bg-muted/80 border-2 border-dashed border-primary/60'
            : 'bg-card/40 border-2 border-transparent',
        )}
      >
        {leads.map((lead) => (
          <LeadCard
        ...
        {isDragOver && leads.length === 0 && (
          <div className="flex-1 flex items-center justify-center py-6">
            <p className="text-xs text-primary">Release to drop</p>
          </div>
        )}
      </div>
      ...
      <Button
        variant="ghost"
        size="sm"
        className="mt-2 w-full justify-start text-muted-foreground/80 hover:text-muted-foreground hover:bg-accent/50 text-xs"
